...
@section('content')
    <div class="page-hero"
        style="background-image: url({{ asset('assets/img/category/' . $category->image->where('type', 'banner')->first()->image) }})">
        <div class="page-hero-content">
            <div class="container">
                <div class="row px-section">
                    <h1 class="page-title text-center text-light">{{ $category->name }}</h1>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row px-section product-category">
        <div class="col-12 col-lg-7">
            <div class="carousel slide" id="carouselCategory">
                @if ($category->image->where('type', 'card')->count() > 1)
            ...
            <div class="sticky-top px-4" style="top:90px; z-index: 20;">
                <ul class="list-group list-group-flush">
                    ...
                    @foreach ($kategoriall as $item)
                        <div class="list-group-item category-list list-group-item-action {{ $item->slug === $category->slug ? 'active' : '' }}"
                            onclick="window.location.href='{{ route('kategori', $item->slug) }}'">{{ $item->name }}
                            <i class="fa fa-caret-right"></i>
                            <div class="subcategory-list">
                                @foreach ($item->subcategory as $sc)
                                    <a class="list-group-item list-group-item-action {{ $sc->slug === $category->slug ? 'active' : '' }}"
